...
# --- 데이터 파싱: 네이버 국내 실시간 금 시세 ---
def get_naver_realtime_gold():
    try:
        ...
        # 실시간 가격 (1g 기준)
        price_text = soup.select_one("div.day_data p.no_today em span.blind").text
        return float(price_text.replace(',', '')) * 3.75 # 1돈 환산
    except:
        return None
...
# --- 데이터 로드: 국내 금 (공공데이터 + 네이버 실시간) ---
@st.cache_data(ttl=600)
def get_krx_merged_data():
    ...
    try:
        ...
        # 네이버 실시간 합치기
        now_price = get_naver_realtime_gold()
        today = datetime.now(KST).replace(hour=0, minute=0, second=0, microsecond=0).replace(tzinfo=None)
        if now_price:
            df_k.loc[today, '종가'] = now_price
            if '등락률' not in df_k.columns: df_k['등락률'] = 0
        return df_k, datetime.now(KST).strftime('%Y-%m-%d %H:%M')
    except: return None, None

# 데이터 실행
df_intl, intl_time = get_intl_data()
df_krx, krx_time = get_krx_merged_data()
...
if df_intl is not None:
    ...
    st.markdown(f'<p class="main-title">🟡 국제 금 시세 (Gold) <span class="ref-time">실시간: {intl_time}</span></p>', unsafe_allow_html=True)
...
if df_krx is not None:
    ...
    st.markdown(f'<p class="main-title">🇰🇷 국내 금 시세 (실시간 반영)</p>', unsafe_allow_html=True)
    st.markdown(f"""<div class="price-box" style="margin-bottom:15px;"><span class="val-sub">국내 현재가 (1돈 기준)</span><span class="val-main">{int(latest_k['종가']):,}원</span><span class="ref-time">네이버 금융 실시간 파싱: {krx_time}</span></div>""", unsafe_allow_html=True)
...
# --- [3] 국제 은 시세 ---
if df_intl is not None:
    st.markdown(f'<p class="main-title">⚪ 국제 은 시세 (Silver) <span class="ref-time">실시간: {intl_time}</span></p>', unsafe_allow_html=True)
    st.markdown(f"""<div class="price-container">
