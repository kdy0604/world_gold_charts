...
def format_delta(curr, prev):
    if curr is None or prev is None or pd.isna(curr) or pd.isna(prev): return ""
    diff = curr - prev
...
def get_safe_data(period="5d", interval="10m"):
    try:
        # 개별 티커 로드 후 병합 (Multi-index 에러 방지)
        g = yf.download("GC=F", period=period, interval=interval, progress=False)['Close']
        s = yf.download("SI=F", period=period, interval=interval, progress=False)['Close']
        e = yf.download("KRW=X", period=period, interval=interval, progress=False)['Close']
        
        df = pd.concat([g, s, e], axis=1)
        ...
        return df_filtered if len(df_filtered) > 2 else df.tail(150)
    except: return None
...
def get_krx_data():
    try:
        ...
        params = {'serviceKey': "ca42a8df54920a2536a7e5c4efe6594b2265a445a39ebc36244d108c5ae9e87a", 'numOfRows': '30', 'resultType': 'xml'}
        res = requests.get(url, params=params, timeout=10)
        root = ET.fromstring(res.text)
        items = root.findall('.//item')
        hist = [{'날짜': pd.to_datetime(i.find('basDt').text), '종가': float(i.find('clpr').text)*3.75, '등락률': float(i.find('flctRt').text or 0)} for i in items]
...
if df_intl is not None and not df_intl.empty and len(df_intl) >= 1:
    c_rt = df_intl.iloc[-1]
    # 일간 데이터가 없을 경우 실시간 데이터로 대체
    c_da = df_daily.iloc[-1] if df_daily is not None else c_rt
    p_da = df_daily.iloc[-2] if df_daily is not None and len(df_daily) >= 2 else c_rt
    ...
    with t1:
        # Y축 자동 최적화 로직 (0부터 시작 방지)
        fig = px.line(df_intl, y='gold_don', template="plotly_white")
        y_min, y_max = df_intl['gold_don'].min() * 0.999, df_intl['gold_don'].max() * 1.001
        fig.update_layout(height=230, margin=dict(l=0,r=0,t=10,b=0), yaxis=dict(range=[y_min, y_max], autorange=False), xaxis_title=None, yaxis_title=None)
        fig.update_traces(line_color='#f1c40f')
        st.plotly_chart(fig, use_container_width=True)
    with t2:
        fig = px.line(df_daily, y='gold_don', template="plotly_white") if df_daily is not None else px.line()
        st.plotly_chart(fig, use_container_width=True)
else:
    st.info("국제 금 시세 데이터를 불러올 수 없습니다. 잠시 후 다시 시도해주세요.")
...
if df_intl is not None and not df_intl.empty:
    ...
    with t3:
        ...
        fig.update_layout(height=230, margin=dict(l=0,r=0,t=10,b=0), yaxis=dict(range=[y_s_min, y_s_max], autorange=False), xaxis_title=None, yaxis_title=None)
        fig.update_traces(line_color='#adb5bd')
        st.plotly_chart(fig, use_container_width=True)
    with t4:
        fig = px.line(df_daily, y='silver_don', template="plotly_white") if df_daily is not None else px.line()
        st.plotly_chart(fig, use_container_width=True)
