...
st.markdown("""
    ...
    .price-box { 
        ...
        text-align: center;
        min-width: 0; /* flex 안에서 텍스트 넘침 방지 */
    }
...
if df_intl is not None:
    ...
    # 2. 국제 금 시세 (모바일 한 줄 고정)
    st.markdown('<p class="main-title">🟡 국제 금 시세 (Gold)</p>', unsafe_allow_html=True)
    st.markdown(f"""
        <div class="mobile-row">
            <div class="price-box">
                <span class="val-sub">국제 (1oz)</span>
                <span class="val-main">${curr["gold"]:,.1f}</span>
                {get_delta_html(curr["gold"], prev["gold"], "$")}
            </div>
            <div class="price-box">
                <span class="val-sub">국내환산 (1돈)</span>
                <span class="val-main">{int(curr["gold_don"]):,}원</span>
                {get_delta_html(curr["gold_don"], prev["gold_don"])}
            </div>
        </div>
        <p class="ref-time-integrated">수집기준: {intl_time} (환율포함)</p>
    """, unsafe_allow_html=True)
...
if df_krx is not None:
    ...
    st.markdown(f"""
        <div class="price-box">
            <span class="val-sub">{"실시간(네이버)" if realtime_kr else "마지막 종가"} (1돈)</span>
            <span class="val-main" style="color:#d9534f; font-size:20px;">{int(disp_p):,}원</span>
            {get_delta_html(disp_p, k_prev)}
        </div>
        <p class="ref-time-integrated">실시간: {naver_time if naver_time else "연결지연"} / 차트: {krx_last_date}</p>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
        <div class="mobile-row">
            <div class="price-box">
                ...
                {get_delta_html(curr["silver"], prev["silver"], "$")}
            </div>
            <div class="price-box">
                <span class="val-sub">국내환산 (1돈)</span>
        ...
        <p class="ref-time-integrated">수집기준: {intl_time}</p>
    """, unsafe_allow_html=True)
